...
internal suspend fun createServer(
    ...
    val server = embeddedServer(CIO, port = auth.config.platformConfig().httpCallbackConfig.httpPort) {
        routing {
            configureRoutes(
                auth,
                onSuccess
    ...
    coroutineScope {
        val timeoutScope = launch {
            ...
            auth.config.platformConfig().urlLauncher.openUrl(
                auth.supabaseClient,
                url(
                    "http://localhost:$port"
                )
            )
            delay(auth.config.platformConfig().httpCallbackConfig.timeout)
            if(this.isActive) {
                auth.logger.d {
